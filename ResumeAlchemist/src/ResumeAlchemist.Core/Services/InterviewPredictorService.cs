...
public class InterviewPredictorService : IInterviewPredictorService
{
    ...
    public InterviewPredictorService(IZhipuAIClient aiClient, ILogger<InterviewPredictorService> logger)
    {
        _aiClient = aiClient;
        _logger = logger;
    }
    ...
    {
        _logger.LogInformation("开始预测面试问题，行业: {IndustryId}, 目标职位: {Position}",
